  ...
  CREATE TABLE IF NOT EXISTS maintenance_history (
    ...
    maintenance_type ENUM('Routine','Repair','Emergency','Inspection','Cleaning','Part Replacement','Other') DEFAULT 'Routine',
    ...
    before_condition ENUM('Excellent','Good','Fair','Poor','Needs Repair') NULL,
    after_condition ENUM('Excellent','Good','Fair','Poor','Needs Repair') NULL,
    before_photo_url VARCHAR(500) NULL,
    after_photo_url VARCHAR(500) NULL,
    notes TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (instrument_item_id) REFERENCES instrument_items(item_id) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (performed_by) REFERENCES users(id) ON DELETE SET NULL ON UPDATE CASCADE,
    INDEX idx_maintenance_item (instrument_item_id),
    INDEX idx_maintenance_type (maintenance_type),
    INDEX idx_maintenance_status (status),
    INDEX idx_maintenance_dates (scheduled_date, completed_date)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
  -- SEED SAMPLE: MAINTENANCE HISTORY
  -- Insert a sample maintenance record for item_id=1 if it doesn't already exist.
  INSERT INTO maintenance_history (
    instrument_item_id, maintenance_type, description, cost, performed_by, performed_by_name,
    scheduled_date, completed_date, next_maintenance_date, status, parts_replaced,
    before_condition, after_condition, notes
  )
  SELECT
    1, 'Inspection', 'Yearly inspection, cleaning and tuning', 500.00, NULL, 'In-house Tech',
    '2024-12-01', '2024-12-02', '2025-12-01', 'Completed', 'Reed set replaced',
    'Good', 'Good', 'Routine annual maintenance'
  FROM DUAL
  WHERE NOT EXISTS (
    SELECT 1 FROM maintenance_history
    WHERE instrument_item_id = 1
      AND maintenance_type = 'Inspection'
      AND scheduled_date = '2024-12-01'
      AND performed_by_name = 'In-house Tech'
  )
  AND EXISTS (SELECT 1 FROM instrument_items WHERE item_id = 1)
  LIMIT 1;

  -- Additional sample maintenance history entries (idempotent)
  INSERT INTO maintenance_history (
    instrument_item_id, maintenance_type, description, cost, performed_by, performed_by_name,
    scheduled_date, completed_date, next_maintenance_date, status, parts_replaced,
    before_condition, after_condition, notes
  ...
  SELECT
    2, 'Repair', 'Replaced damaged valve and adjusted tuning slide', 1500.00, NULL, 'External Repair Shop',
    '2023-05-10', '2023-05-12', '2024-05-10', 'Completed', 'Valve assembly',
    'Fair', 'Good', 'Customer reported sticking valve during use'
  FROM DUAL
  WHERE NOT EXISTS (
    SELECT 1 FROM maintenance_history
    WHERE instrument_item_id = 2
      AND maintenance_type = 'Repair'
      AND scheduled_date = '2023-05-10'
  )
  AND EXISTS (SELECT 1 FROM instrument_items WHERE item_id = 2)
  LIMIT 1;
  ...
  SELECT
    3, 'Routine', 'Lubrication and headjoint alignment; general tuning', 300.00, NULL, 'In-house Tech',
    '2022-11-20', '2022-11-20', '2023-11-20', 'Completed', NULL,
    'Good', 'Good', 'No issues found'
  FROM DUAL
  WHERE NOT EXISTS (
    SELECT 1 FROM maintenance_history
    WHERE instrument_item_id = 3
      AND maintenance_type = 'Routine'
      AND scheduled_date = '2022-11-20'
  )
  AND EXISTS (SELECT 1 FROM instrument_items WHERE item_id = 3)
  LIMIT 1;
  ...
  SELECT
    4, 'Part Replacement', 'Replaced worn mouthpiece and cleaned interior tubing', 600.00, NULL, 'In-house Tech',
    '2024-02-15', '2024-02-16', '2025-02-15', 'Completed', 'Mouthpiece',
    'Good', 'Good', 'Regular wear on mouthpiece after rental season'
  FROM DUAL
  WHERE NOT EXISTS (
    SELECT 1 FROM maintenance_history
    WHERE instrument_item_id = 4
      AND maintenance_type = 'Part Replacement'
      AND scheduled_date = '2024-02-15'
  )
  AND EXISTS (SELECT 1 FROM instrument_items WHERE item_id = 4)
  LIMIT 1;
  ...
  SELECT
    6, 'Inspection', 'Pre-performance check and snare tension adjustment', 250.00, NULL, 'Drum Tech Co.',
    '2023-07-01', '2023-07-01', '2024-07-01', 'Completed', NULL,
    'Good', 'Good', 'Adjusted snares and replaced a bad rim clamp'
  FROM DUAL
  WHERE NOT EXISTS (
    SELECT 1 FROM maintenance_history
    WHERE instrument_item_id = 6
      AND maintenance_type = 'Inspection'
      AND scheduled_date = '2023-07-01'
  )
  AND EXISTS (SELECT 1 FROM instrument_items WHERE item_id = 6)
  LIMIT 1;
  ...
  SELECT
    7, 'Repair', 'Repaired cracked shell and reseated lugs', 1800.00, NULL, 'Matina Repair Shop',
    '2024-08-22', '2024-08-27', '2025-08-22', 'Completed', 'Shell patch, lug screws',
    'Poor', 'Fair', 'Damage from transport; replaced hardware as necessary'
  FROM DUAL
  WHERE NOT EXISTS (
    SELECT 1 FROM maintenance_history
    WHERE instrument_item_id = 7
      AND maintenance_type = 'Repair'
      AND scheduled_date = '2024-08-22'
  )
  AND EXISTS (SELECT 1 FROM instrument_items WHERE item_id = 7)
  LIMIT 1;
  ...
  CREATE TABLE IF NOT EXISTS condition_assessments (
    ...
    instrument_item_id INT NOT NULL,
    assessment_type ENUM('checkout','checkin','inspection','damage_report') NOT NULL,
    related_rental_id INT NULL,
    related_borrow_id INT NULL,
    condition_status ENUM('Excellent','Good','Fair','Poor','Needs Repair') NOT NULL,
    ...
    INDEX idx_assessment_item (instrument_item_id),
  ...
  -- Add foreign keys for condition assessments 
  SET @fk_exists = (
    SELECT COUNT(*) FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE

    WHERE table_schema = DATABASE() 
    AND table_name = 'condition_assessments' 
    AND CONSTRAINT_NAME = 'fk_assessment_rental'
  );
  SET @stmt = IF(@fk_exists = 0,
    'ALTER TABLE condition_assessments ADD CONSTRAINT fk_assessment_rental FOREIGN KEY (related_rental_id) REFERENCES rent_requests(request_id) ON DELETE SET NULL',
    'SELECT 1');
  ...
  SET @fk_exists = (
    ...
    WHERE table_schema = DATABASE() 
    AND table_name = 'condition_assessments' 
    AND CONSTRAINT_NAME = 'fk_assessment_borrow'
  );
  SET @stmt = IF(@fk_exists = 0,
    'ALTER TABLE condition_assessments ADD CONSTRAINT fk_assessment_borrow FOREIGN KEY (related_borrow_id) REFERENCES borrow_requests(request_id) ON DELETE SET NULL',
    'SELECT 1');
  PREPARE st FROM @stmt; EXECUTE st; DEALLOCATE PREPARE st;

  CALL add_index_if_missing('condition_assessments','idx_assessment_rental','ALTER TABLE condition_assessments ADD INDEX idx_assessment_rental (related_rental_id)');
  CALL add_index_if_missing('condition_assessments','idx_assessment_borrow','ALTER TABLE condition_assessments ADD INDEX idx_assessment_borrow (related_borrow_id)');
  ...
  -- ============================================================================
  -- SEED DATA: BORROW & RENT REQUESTS
  -- ============================================================================
  ...
  -- ============================================================================
  -- SEED DATA: INSTRUMENT ITEMS (Serial Number Tracking)
  -- ============================================================================
  -- Ensure explicit instrument_items exist for key seeded instruments
  -- (Add serial_number and acquisition_date where missing). These inserts are
  -- idempotent: they only run when the specific serial_number doesn't already
  -- exist. They use the canonical combined location so the UI shows a real site.
