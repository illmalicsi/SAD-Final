...
(async () => {
  ...
  try {
    ...
    console.log('\n5) Sample 20 instruments: instrument_id, name, price_per_day, condition_status, availability_status, quantity');
    try {
      const [sample] = await conn.query('SELECT instrument_id, name, price_per_day, condition_status, availability_status, quantity FROM instruments ORDER BY instrument_id LIMIT 20');
      console.log(sample);
    } catch (err) {
